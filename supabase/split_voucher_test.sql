-- Transaction-only Split Voucher verification.
--
...
begin
  ...
  from public.create_parcel_with_counter(
    ...
    p_cash_advance := 0,
    p_remark := 'Split parent verification',
    p_created_at := now()
  );
  ...
  v_split_result := public.split_parcel(
    v_parent.id,
    jsonb_build_array(
      jsonb_build_object(
        'number_of_parcels', 2,
  ...
  if v_parent.status <> 'split' then
    raise exception 'Expected parent status split, got %', v_parent.status;
  end if;

  if v_parent.split_count <> 2 then
    raise exception 'Expected parent split_count 2, got %', v_parent.split_count;
  end if;
  ...
  if v_child_count <> 2 then
    raise exception 'Expected exactly 2 split children';
  end if;
  ...
  if v_child_a.number_of_parcels <> 2 or v_child_b.number_of_parcels <> 2 then
    raise exception 'Expected child quantities 2 and 2, got % and %',
      v_child_a.number_of_parcels, v_child_b.number_of_parcels;
  end if;

  if v_child_a.total_charges <> 11111 or v_child_b.total_charges <> 22222 then
    raise exception 'Manual child charges were not preserved: % and %',
      v_child_a.total_charges, v_child_b.total_charges;
  end if;

  if v_child_a.cash_advance <> 101 or v_child_b.cash_advance <> 202 then
    raise exception 'Manual child cash advances were not preserved: % and %',
      v_child_a.cash_advance, v_child_b.cash_advance;
  end if;
  ...
  raise notice
    'OK split voucher test: parent %, children %, %, manual charges %, %, parent attach rejected, child attach accepted',
