...
- Document and implement Split Voucher server RPC later: add additive parcel split columns/status, create child vouchers atomically on the server, update Android Parcel Detail with Split Voucher UI, and update Windows Ledger/Incoming attach guards.
...
- Split child tracking IDs must be generated by a future authenticated server RPC, not locally. Format is `PARENT-A`, `PARENT-B`, etc.
