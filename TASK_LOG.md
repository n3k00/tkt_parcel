...
- Changed new-device initial parcel pull to latest-first paging. First sync now fetches only the newest visible Supabase page, tracks `parcel_pull_last_synced_at` for future incremental refreshes, and tracks `parcel_pull_oldest_loaded_at` for Parcel History's Load Older backfill.
